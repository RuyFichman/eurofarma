...
- Infraestrutura de WhatsApp independente de provedor, com adaptadores para a Cloud API da Meta e para o Twilio, validação de assinatura nos respectivos webhooks, rate limiting local, máquina de estados compartilhada e simulador local. O fluxo ativo já oferece menu, FAQ, elegibilidade por CEP ou município, orientação dentro ou fora da área, cadastro simplificado opcional com consentimento, retomada pelo `JourneyStatus` e contato transparente com o Lactare. O consentimento vem antes da solicitação e gravação do nome; o CEP não é persistido, marketing permanece desligado, lembretes começam desligados e só mudam por escolha explícita, e o webhook não cria novos agendamentos. Antes de qualquer efeito, a tabela `whatsapp_inbound_messages` reivindica a combinação exclusiva de provedor e identificador, registra recebimento, conversa e resultado sem corpo ou PII e encerra reentregas sem repetir cadastro, consentimento, transição ou resposta. A Cloud API usa `wamid` e o Twilio usa o `MessageSid`; a migration `20260918100000_add_whatsapp_inbound_messages` está local e ainda não foi aplicada no Supabase cloud. A migration conversacional está aplicada no Supabase cloud.
...
- Base do RF17 com opt-in específico para avisos de status no cadastro web, separado dos lembretes. A mesma transação que altera o `JourneyStatus` e grava o histórico também cria a outbox; cada histórico aceita no máximo um aviso por chave idempotente. Sem consentimento vigente, o item nasce suprimido. O processador usa claim concorrente com lock recuperável, revalida consentimento e exclusão lógica, aplica backoff e limite de tentativas e mantém auditoria append-only de cada resultado. O simulador exercita sucesso, falha transitória e falha permanente sem chamar provedor nem imprimir PII. O job `pnpm whatsapp:outbox:twilio` usa a Programmable Messaging API: texto livre somente até 24 horas após a última mensagem recebida e, fora da janela, template aprovado por `ContentSid`; o `MessageSid` retorna para a auditoria existente. Credenciais, Messaging Service e templates aprovados continuam necessários para entrega real. O `POST /api/whatsapp/twilio/status` valida callbacks e acrescenta `MessageSid`, status categórico e código de erro em `notification_delivery_status_events`, sem corpo, telefone ou PII; a auditoria é append-only e pode manter callback válido ainda sem vínculo com a outbox. A migration `20260918110000_add_notification_delivery_status_events` está local e ainda não foi aplicada no Supabase cloud. Esses eventos habilitam evolução futura do dashboard para envio, entrega e leitura efetivos sem expor PII. A migration `20260916180000_add_notification_outbox` foi aplicada no Supabase cloud em 16 de setembro de 2026.
- Consentimento de lembretes separado dos avisos de status, com opt-in no cadastro e ativação ou cancelamento posterior na área autenticada e no chatbot. A ativação exige uma data de referência sem horário; cada mudança acrescenta um evento auditável ao ledger append-only, e reentregas do WhatsApp são idempotentes pelo id da mensagem. O job de enfileiramento reutiliza a outbox do RF17: dois dias após `KIT_SENT`, enquanto essa etapa continua atual e o opt-in está vigente, cria um único lembrete de continuidade com payload mínimo e chave idempotente, sem representar agendamento ou confirmação. A finalidade vem da migration `20260916190000_add_reminder_consent_purpose`, aplicada no Supabase cloud em 16 de setembro de 2026; as migrations do item `REMINDER` também foram aplicadas no mesmo dia. As migrations locais `20260916220000_add_reminder_reference_date` e `20260916221000_add_reminder_reference_step` aguardam aplicação no Supabase cloud.
- A suíte completa passa contra o Supabase cloud com 580 testes em 82 arquivos, incluindo os testes da outbox e do consentimento de lembretes.
