...
- **Rate limiting:** `POST /api/nutriz` já limita 5/min por IP (`lib/security/rate-limit.ts`, Sprint 4.4) e o login admin tem lockout de 5 falhas/5min por email (`lib/auth/login-rate-limit.ts`, Sprint 5.2). Ambos são **em memória, por processo** — trocar por store distribuído antes de deploy multi-instância. `api/track` ainda não existe (Sprint 3.8); quando nascer, limitar também.
