...
- A atualização administrativa da jornada foi consolidada localmente em seis marcos visíveis, iguais aos da experiência pública: primeiro contato pelo WhatsApp, verificação da área atendida, ficha de saúde e exame de sangue, entrega do kit em casa, extração e armazenamento em casa e coleta domiciliar do leite. O dropdown sempre apresenta os seis marcos nessa ordem e permite ao admin avançar, retornar ou registrar novamente o status atual; cada mudança permanece no histórico append-only. Os valores antigos do enum permanecem somente para compatibilidade com perfis e históricos existentes; o painel não os oferece como novas opções. As migrations `20260920120000_consolidate_admin_journey_statuses` e `20260920130000_allow_any_admin_journey_status` atualizam o CHECK de `journey_status_history`; a segunda o torna permissivo por `to_status` (qualquer um dos seis marcos, de qualquer origem, `NOT VALID` para não recatalogar histórico legado). **Incidente de 21 de setembro de 2026:** as duas ficaram só commitadas no repositório — nunca chegaram a ser aplicadas no Supabase cloud —, então o painel oferecia transições (ex.: Verificação da área → Ficha de saúde e exame) que o CHECK ainda vigente recusava, e todo `PATCH` de status via `/admin/nutrizes/[id]` falhava com o erro genérico de banco. Aplicadas agora pelo MCP do Supabase, nessa ordem; a definição da constraint foi conferida direto no banco (`to_status IN (...) NOT VALID`, `convalidated: false`). O teste de integração `tests/integration/admin-nutriz-journey.test.ts` também estava desatualizado — cobrava o CHECK anterior (`120000`), não o vigente (`130000`) — e foi reescrito para exercitar o comportamento final: qualquer origem é aceita, só o destino é restrito aos seis marcos. Pendente: as duas migrations ainda não foram registradas em `_prisma_migrations` (checksums SHA-256: `06c14cf5f4b9928640141601d89828ea796f7025b155a2c4bf24156b0e369631` para a `120000` e `42e9f2967fc88f2a2833adeedc351a3702b59e7b9afac9f8f47818ccf5ad5f60` para a `130000`) — o `INSERT` foi bloqueado pelo classificador de permissão do Claude Code por escrever em produção; precisa de aprovação explícita do time para rodar.
...
| RF06 — lembretes opcionais | **Parcial, consentimento e configuração implementados.** Opt-in e cancelamento são separados, opcionais e auditáveis no chatbot e, desde 20 de setembro de 2026, na área autenticada via "Meus lembretes"; o cadastro web deixou de oferecer esse opt-in em 21 de setembro de 2026 (três tipos configuráveis individualmente: validade do leite, doação futura, entrega do kit). Nenhum deles agenda ou confirma nada. O job que calcularia quando cada lembrete vence e o enfileiraria na outbox ainda não existe (ver "Meus lembretes" na seção 3); a entrega real continua dependendo da Meta. |
...
| RF08 — painel autenticado | **Implementado.** Inclui checagem de role ADMIN. Desde 26 de setembro de 2026, o dashboard abre com a Central de Ação ("O que precisa de atenção hoje"), com filas operacionais e detalhamento em `/admin/atencao/[fila]` (seção 3, "Central de Ação do painel"). |
...
| RF12 — cartão de impacto | **Implementado localmente como “Postal do Bem”.** Depois de ao menos uma transição `DONATION_CONFIRMED` registrada pela equipe do Lactare, a Minha Área libera uma imagem universal em PNG, sem nome, contagem, dado pessoal ou alegação clínica. A nutriz pode abrir o compartilhamento nativo do dispositivo — incluindo WhatsApp e redes sociais compatíveis — ou baixar a imagem. |
...
- Rascunho de cadastro do chatbot sem prazo de retenção: `whatsapp_conversations.context.registration` guarda nome, CPF, e-mail e endereço coletados **antes** do consentimento e só é apagado quando a nutriz aceita ou recusa. Uma conversa abandonada no meio do cadastro mantém esses dados indefinidamente. Identificado em 26 de setembro de 2026, na descoberta da Central de Ação (que por isso não tem fila de "cadastros incompletos"); falta definir uma expiração e um job de limpeza.
...
4. Definir a evidência operacional definitiva da confirmação de doação antes de calcular impacto, contar recorrência institucional ou criar reconhecimento de indicação. O “Postal do Bem” já usa somente o status administrativo `DONATION_CONFIRMED` como gate e não quantifica impacto. A mensagem de indicação usa somente o link próprio já atribuído e não depende de confirmação de doação; os reconhecimentos objetivos por status permanecem limitados ao status administrativo registrado.
...
7. Evoluir a Central de Ação depois de validar com o Lactare os limites provisórios e o responsável de cada fila: estado "resolvido/adiado" por item (principalmente para encerrar um atendimento humano pelo painel), modelo de atribuição a pessoas e, se desejado, aviso à equipe quando uma fila chegar a criticidade alta.

### Atualização da Minha Área (19 de setembro de 2026)

A área pessoal foi reorganizada em seções empilhadas de largura total: jornada, registro de extração, lembretes, histórico de doações, destaques e "Meus dados". O histórico de doações lê apenas as transições `DONATION_CONFIRMED` e `RECURRING_DONATION_ELIGIBLE` já registradas pelo Lactare e concentra a exportação do PDF, que antes vivia num card próprio (`personal-history-card.tsx`, removido). O destaque “Postal do Bem” usa somente a existência de `DONATION_CONFIRMED` como gate: libera uma arte estática universal para compartilhamento nativo ou download, sem personalização, quantificação de impacto ou exposição de dados da nutriz.
...
### Central de Ação do painel (26 de setembro de 2026)

O dashboard (`/admin/dashboard`) passou a abrir com **"O que precisa de atenção hoje"**, acima dos filtros e dos indicadores, que continuam iguais. A seção transforma o painel de relatório em ferramenta operacional: cada fila mostra quantidade, criticidade, espera mais longa e mediana, responsável e ação recomendada, com link para o detalhamento paginado em `/admin/atencao/[fila]` (20 por página, do item mais antigo para o mais recente). Cada item ligado a uma nutriz leva a `/admin/nutrizes/[id]`. As filas não respondem aos filtros de segmentação do dashboard, porque o objetivo é mostrar tudo o que pede ação. Filas vazias viram uma única linha de "em dia".

Filas entregues, com as fontes:

| Fila (slug) | Fonte e início da espera | Entra na fila |
|---|---|---|
| Pedidos de atendimento humano (`atendimento-humano`) | `whatsapp_conversations` com `step = HUMAN_HANDOFF`. Não existe coluna de entrada no passo: a espera parte de `last_message_at`, que fica congelado durante a pausa porque as mensagens seguintes são marcadas `IGNORED` e não chamam `saveConversationState` (`lib/whatsapp/process-inbound.ts`). Conta só a janela de atendimento humano do Lactare (seg–sáb, 9h–18h, Brasília, UTC-3 fixo). | Sempre |
| Nutrizes sem avanço registrado (`sem-avanco`) | Último `journey_status_history.changed_at`, ou `created_at` sem histórico. Status de `REGISTERED` a `AWAITING_RESULT`. | 14 dias |
| Kits enviados sem entrega registrada (`kit-sem-entrega`) | Entrada em `KIT_SENT`; também `kit_delivery_scheduled_at` vencido há mais de 1 dia, que eleva a criticidade para, no mínimo, média. | 7 dias, ou visita vencida |
| Falhas de envio (`falhas-de-envio`) | Outbox `FAILED`; último status por mensagem em `notification_delivery_status_events` igual a `FAILED`/`UNDELIVERED`; `whatsapp_inbound_messages` com `FAILED` ou presas em `PROCESSING` há mais de 15 min. Janela de 7 dias, porque falha não tem estado "resolvida". | Na ocorrência |
| Kit entregue sem doação registrada (`primeira-doacao`) | Entrada em `KIT_DELIVERED` (+ legado `ELIGIBLE`) sem nenhuma `DONATION_CONFIRMED` no histórico. | 21 dias |
| Possíveis doadoras recorrentes sem retorno recente (`doadoras-sem-retorno`) | Última `DONATION_CONFIRMED` no histórico, com status atual `DONATION_CONFIRMED`/`RECURRING_DONATION_ELIGIBLE`. Nunca chega a criticidade alta: é convite, não pendência. | 60 dias |

As quatro filas de jornada são disjuntas por status atual (`PROFILE_QUEUE_STATUSES`), então uma nutriz aparece no máximo em uma; `NOT_ELIGIBLE` e perfis com soft delete nunca entram. Os avisos da outbox em `PENDING`/`RETRY_SCHEDULED`/`PROCESSING` **não** contam como falha: sem a integração com a Meta nada os envia, e contá-los manteria um alarme permanente. Eles aparecem só como linha informativa ("aguardando a ativação do envio pelo WhatsApp"), por decisão do time.

**Criticidade e limites.** Alta/média/baixa derivam **somente do tempo de espera**, nunca de dado clínico, exame ou aptidão, e são sempre exibidas com ícone e texto. Os limites vivem em `lib/admin/action-center/thresholds.ts`, em minutos, e são **provisórios, pendentes de validação com a equipe do Lactare**: atendimento humano média ≥ 2 h e alta ≥ 4 h de expediente; sem avanço média 21 e alta 30 dias; kit média 10 e alta 14 dias; falhas média na entrada e alta ≥ 24 h; primeira doação média 30 e alta 45 dias; recorrentes média 90 dias. As consultas recebem cortes calculados a partir dessas constantes (`cutoffs.ts`); nenhum número fica dentro do SQL.

**Responsável.** Não existe modelo de atribuição. Cada fila tem um papel fixo no i18n ("Equipe de atendimento do Lactare", "Equipe operacional do Lactare", "Suporte técnico do NutriLink, com a equipe do Lactare"), e a interface diz explicitamente que é uma equipe, não uma pessoa.

**Exposição de dados.** O resumo é só agregado: nenhuma consulta dele lê nome, telefone, e-mail, CPF ou endereço. O detalhamento lê nome, cidade/UF e status atual — menos que `/admin/nutrizes`, que também mostra contato. A fila de atendimento humano nunca exibe telefone: conversa sem cadastro aparece como "Conversa sem cadastro", localizável pelo horário no WhatsApp do Lactare. A fila de falhas mostra só categoria, origem e código técnico. CPF, endereço, e-mail, `context` da conversa e corpo de mensagem nunca são selecionados. Slugs de URL são fixos e validados com Zod; slug desconhecido vira 404.

**Arquitetura.** Regras puras e Prisma-free em `lib/admin/action-center/` (limites, criticidade, espera corrida e em horário comercial, partição por status, slugs, ordenação); consultas em `lib/db/queries/admin-action-center.ts`, agregadas no PostgreSQL, sem listas de ids nem perfis em memória. A única exceção limitada é o handoff: o banco devolve até 1.000 instantes de início de pausa, sem telefone ou nome, porque a espera em horário comercial é calculada em TypeScript. Os componentes em `components/admin/action-center/` são todos Server Components. `app/admin/(painel)/atencao/[fila]/load.ts` repete `requireAdminUser()` antes de qualquer consulta, além do gate do layout, porque layout e página renderizam em paralelo e a tela lista pessoas. **Nenhuma migration**: os índices existentes cobrem as consultas no volume atual. Um índice em `whatsapp_conversations (step, last_message_at)` só será útil em escala.

**Ficou de fora, deliberadamente:**
- **Cadastros incompletos no chatbot.** O rascunho em `context.registration` guarda nome, CPF, e-mail e endereço **sem consentimento**. Contá-los não leva a uma ação legítima e incentivaria contato com quem não consentiu. O problema real é de retenção (seção 3.4).
- **Consentimentos e exclusões.** Retiradas de opt-in já são suprimidas automaticamente pela outbox, e o procedimento de remoção definitiva após soft delete não está definido; não havia ação operacional a propor.
- **Estado "resolvido/adiado" por item e atribuição a pessoas.** O handoff sai da fila só quando a nutriz volta ao menu — o painel não sabe se a equipe já respondeu —, e falhas saem só pela janela de 7 dias.

Testes: `tests/unit/admin-action-center.test.ts` (espera em horário comercial, virada de dia UTC, domingo, sem horário de verão, criticidade, ordenação, partição por status, slugs, paginação e copy sem "abandono"/"doadora ativa"/promessa de coleta), `tests/unit/admin-action-center-page.test.ts` (não admin é barrado antes da consulta; slug desconhecido vira 404) e `tests/integration/admin-action-center.test.ts`. Este último roda numa transação sempre revertida, porque histórico e auditoria são append-only, e compara com uma linha de base da mesma transação, porque o cloud tem dados reais. Ele confere o que entra e o que não entra em cada fila e que nenhuma saída contém CPF, e-mail, endereço ou telefone.
...
| Testes | Vitest | Última suíte completa no cloud: 781 em 109 arquivos (26/09/2026). |
...
A máquina de estados já cobre localmente os itens 1 a 7, inclusive retomada pelo status categórico registrado pelo Lactare e ativação ou cancelamento de lembretes por uma nutriz cadastrada. O handoff humano local também está implementado: pedido explícito, mesmo chat, pausa do bot e resposta distinta dentro ou fora do expediente. O item 8 tem base local de outbox, mas ainda não entrega pela Meta; o reconhecimento derivado de indicação permanece pendente, assim como o vídeo institucional, a operação humana conectada à infraestrutura real da Meta e a própria infraestrutura da Meta. O “Postal do Bem” universal já está disponível na Minha Área após `DONATION_CONFIRMED`, sem envio pelo chatbot nem alegação de impacto. A mensagem de indicação é disponibilizada na Minha Área, e não enviada pelo chatbot. As migrations conversacional, da outbox e da finalidade de lembretes estão aplicadas no Supabase; a migration `20260916200000_add_human_handoff_step` também está aplicada e registrada. Não chamar RF11 de concluído.

Atualização do UC14: o “Postal do Bem” universal é liberado exclusivamente na Minha Área depois de `DONATION_CONFIRMED`; não contém PII, contagem ou alegação clínica. O reconhecimento derivado de indicação continua pendente de evento próprio. A mensagem de indicação também está disponível exclusivamente na Minha Área: ela é editável, contém somente o link opaco da própria nutriz e pode ser copiada ou aberta pré-preenchida no WhatsApp. O chatbot não envia essas peças, não registra destinatários e não concede recompensa material.
