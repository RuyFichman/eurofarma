...
- Adaptador local de teste para a Z-API: `zapi-provider` envia texto, botões e listas; `POST /api/whatsapp/zapi/[secret]` aceita somente callbacks privados `ReceivedCallback`, confere segredo opaco, `instanceId`, formato e a allowlist obrigatória quando `ZAPI_TEST_MODE=true`, e descarta mensagens próprias, grupos, canais, status, edições e mídia antes de reutilizar a máquina de estados e sua idempotência. O segredo é necessário porque a documentação consultada da Z-API não descreve assinatura do callback. A migration `20260919120000_add_zapi_whatsapp_provider` acrescenta `ZAPI` ao enum de entradas idempotentes. A migration local `20260919130000_add_zapi_delivery_audit` acrescenta `ZAPI` à auditoria de status e grava, na própria entrada idempotente, somente o resultado técnico da resposta reativa, seu id do provedor e código de erro categórico — nunca corpo, telefone ou PII. Se o envio falha, a entrada termina como `FAILED`, sem repetir os efeitos. `POST /api/whatsapp/zapi/status/[secret]` mapeia os callbacks `SENT`, `RECEIVED` e `READ` para a auditoria append-only. O adaptador não habilita outbox de lembretes ou avisos proativos, que continua dependente de decisão operacional e de transporte apropriado.
