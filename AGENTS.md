...
"Seus selos" virou "Meus selos" (`my-badges-card.tsx`, extraído de `personal-highlights.tsx`) e, na mesma data, ganhou o mesmo padrão de disclosure nativo (`details`/`summary`, sem Client Component) de "Minha jornada": fechado, mostra só a última conquista e a ação "Ver selos"; aberto, lista os cinco `NutrizRecognitionKind` possíveis (`RECOGNITION_KIND_ORDER`, novo em `lib/journey/recognitions.ts`) lado a lado, cada um com ícone, título, descrição e "Conquistado em {data}" ou "Ainda não conquistado", além do contador "X/5 conquistados até agora". "Cartão de impacto" permanece ao lado, sem mudança; a grade ganhou `md:items-start` para não esticar esse card quando "Meus selos" abre. O mockup de referência trazia selos por contagem de doações e por indicação ("Coração generoso", "Fonte que não seca", "Corrente do bem" com barra de progresso), mas isso exigiria novos valores de `NutrizRecognitionKind`, migration e lógica de contagem que ainda não existem — RF12 e o reconhecimento de indicação continuam pendentes de fonte legítima (seção 3.4). Esta mudança ficou só na interação de abrir/fechar e no rótulo, usando os cinco reconhecimentos já implementados.
