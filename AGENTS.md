...
- A atualização administrativa da jornada foi consolidada localmente em seis marcos visíveis, iguais aos da experiência pública: primeiro contato pelo WhatsApp, verificação da área atendida, ficha de saúde e exame de sangue, entrega do kit em casa, extração e armazenamento em casa e coleta domiciliar do leite. Os valores antigos do enum permanecem somente para compatibilidade com perfis e históricos existentes; o painel não os oferece como novas opções. A migration `20260920120000_consolidate_admin_journey_statuses` atualiza o CHECK de transições e ainda precisa ser aplicada no Supabase cloud.
