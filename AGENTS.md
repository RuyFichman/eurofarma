...
TypeScript estrito está ativo com strict e noUncheckedIndexedAccess. A última suíte completa validada no Supabase passa com **498 testes em 58 arquivos**: 421 unitários e 77 de integração. As implementações locais do RF17 e do consentimento de lembretes elevaram a suíte unitária para **453 testes em 54 arquivos**, todos passando; os testes de integração novos dependem da aplicação das migrations `20260916180000_add_notification_outbox` e `20260916190000_add_reminder_consent_purpose` no Supabase cloud. As migrations anteriores do RF07, RF16, estado conversacional e `service_municipalities` continuam aplicadas.
...
- Base local do RF17 com consentimento específico para avisos de status no cadastro web e outbox criada na mesma transação da mudança de jornada. Cada histórico gera no máximo um item por chave idempotente; sem opt-in vigente, ele nasce `SUPPRESSED`. O processador usa claim concorrente com lock recuperável, revalida consentimento e soft delete antes do envio, aplica backoff, limita tentativas e grava auditoria append-only de sucesso, nova tentativa, falha ou supressão. `pnpm whatsapp:outbox:sim` exercita sucesso e falhas sem chamar a Meta nem imprimir PII. A migration `20260916180000_add_notification_outbox` ainda precisa ser aplicada e registrada no Supabase; até isso ocorrer, RF17 não funciona contra o banco cloud.
- Consentimento local de lembretes separado dos avisos de status. A nutriz pode optar no cadastro e ativar ou cancelar depois pela área autenticada ou pelo chatbot. Cada mudança acrescenta um evento `GRANTED` ou `WITHDRAWN` ao ledger append-only, com canal, versão da política e horário; reentregas do WhatsApp usam o id da mensagem como chave idempotente. Nenhum job ou mensagem de lembrete foi criado, e os textos deixam explícito que lembrete não é agendamento nem confirmação. A finalidade `REMINDERS_WHATSAPP` depende da migration `20260916190000_add_reminder_consent_purpose`, ainda não aplicada no Supabase.
...
| RF06 — lembretes opcionais | **Parcial, consentimento implementado localmente.** Opt-in e cancelamento são separados, opcionais e auditáveis no cadastro, na área autenticada e no chatbot. A migration da finalidade ainda precisa ser aplicada; o job e o envio dos lembretes continuam pendentes. |
...
| RF17 — aviso de mudança de status | **Parcial, implementado localmente.** A mudança válida de status cria histórico e outbox atomicamente, com idempotência, consentimento específico, tentativas, backoff, auditoria e simulador local. A migration da outbox ainda precisa ser aplicada no Supabase, e a entrega real depende de infraestrutura e templates da Meta. |
...
- Aplicação da migration do consentimento e job de lembretes.
...
2. Implementar retirada/reconcessão do opt-in de avisos na área autenticada e no chatbot; o modelo append-only já suporta os dois eventos.
3. Aplicar a migration do consentimento de lembretes e implementar o job de envio sem semântica de agendamento; concluir também o handoff humano operacional.
...
| Testes | Vitest | Última suíte completa no cloud: 498 em 58 arquivos. Com RF17 e consentimento de lembretes locais: 453 unitários em 54 arquivos; integrações novas pendentes das migrations. |
